...
{
	public class TEMPORARY_PacuRainEvent() : TwitchEventBase(ID)
	{
		public const string ID = "PacuRain2";
		...
		{
			return Mod.isPacuUWUHere ?
				Strings.Get("STRINGS.ONITWITCH.EVENTS.RAIN_PREFAB_PACU_PACUWU")
				: Strings.Get("STRINGS.ONITWITCH.EVENTS.RAIN_PREFAB_PACU_NORMAL");
		}
		...
		public override void Run()
		{
			...
			var go = new GameObject("AETE_PacuSpawner");
			var spawner = go.AddComponent<ThingSpawner>();
			spawner.transform.position = startPosition;
			spawner.minCount = 10;
			spawner.maxCount = 10;
			spawner.prefabTags = [PacuConfig.ID, PacuTropicalConfig.ID, PacuCleanerConfig.ID];
			spawner.radius = 4;
			spawner.minDelay = 0.5f;
			spawner.maxDelay = 1f;
			spawner.soundFx = ModAssets.Sounds.POP;
			spawner.sceneLayer = Grid.SceneLayer.Creatures;
			spawner.spawnFx = ModAssets.Fx.pinkPoof;
			...
			ToastManager.InstantiateToast(
				...
				string.Format(
					...
					Util.StripTextFormatting(global::STRINGS.CREATURES.SPECIES.PACU.NAME)
				)
			);
		}
	}
